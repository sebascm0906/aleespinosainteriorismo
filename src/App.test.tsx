...
test('mantiene el contrato visual de los marcos de comparación y del panel móvil', () => {
  ...
  expect(css).toMatch(/\.carrusel \.transformation-pair figure > picture\s*\{[^}]*aspect-ratio:\s*1165 \/ 1040/)
  expect(css).toMatch(/\.carrusel \.transformation-pair figure > picture\s*\{[^}]*display:\s*block/)
  expect(css).toMatch(/\.carrusel \.transformation-pair figure > picture\s*\{[^}]*width:\s*100%/)
  expect(css).not.toMatch(/\.carrusel \.transformation-pair figure > picture\s*\{[^}]*height:\s*100%/)
  expect(css).toMatch(/\.carrusel \.transformation-pair figure > picture\s*\{[^}]*overflow:\s*hidden/)
  ...
  expect(mobileCss).toMatch(/\.site-header\s+\.mobile-navigation[^{}]*\{[^}]*grid-column:\s*1\s*\/\s*-1/)
